...
async function getFiltered() {
  ...
  if (country !== "all") rows = rows.filter((c) => c.pais === country);

  if (q.trim()) {
    const qq = q.trim().toLowerCase();
    rows = rows.filter(
      (c) =>
        c.nombre.toLowerCase().includes(qq) || c.pais.toLowerCase().includes(qq)
    );
  }
...
async function render() {
  ...
  if (view === "cards") await renderCards();
...
async function openDetail(id) {
  ...
  history.replaceState(
    null,
    "",
    `#circuit=${encodeURIComponent(c.idCircuito)}`
  );
