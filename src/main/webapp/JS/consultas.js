...
async function initFilters() {
    ...
    /*const raceSel = $("#fRace");
     dataHistorialCarreras.filter(r => r.carrera.temporada.anio === "2025").forEach(r=>{
     const o = document.createElement("option"); o.value=r.carrera.circuito.nombre; o.textContent=`${r.carrera.circuito.nombre} ${r.idCarrera.temporada.anio}`; raceSel.appendChild(o);
     });*/
    const races = unique(dataHistorialCarreras.map(r => r.carrera.circuito.nombre)).filter(n => n !== "—").sort();
    const drivers = unique(dataHistorialCarreras.map(r => r.piloto.nombre)).filter(n => n !== "—").sort();
    const teams = unique(dataHistorialCarreras.map(r => r.piloto.escuderia.nombre)).filter(n => n !== "—").sort();

    const dSel = $("#fDriver"), tSel = $("#fTeam"), rSel = $("#fRace");
    drivers.forEach(n => {
...
async function renderTable() {
    ...
    tb.innerHTML = "";
    rows.forEach(r => {
        /*const raceName = rows.find(x=>x.idHistorial===r.idCarrera)?.idCircuito.nombre || r.idCarrera;*/
...
function buildSeriesForTarget() {
    ...
    if (!key)
        return {labels: [], hist: [], pred: [], p10: [], p90: []};
...
function bindEvents() {
    ...
    $("#btnCSV").addEventListener("click", async () => {
        ...
        downloadCSV("consultas.csv", csvRows);
    });
}
